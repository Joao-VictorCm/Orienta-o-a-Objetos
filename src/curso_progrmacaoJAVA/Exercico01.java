package curso_progrmacaoJAVA;

import java.util.Locale;

public class Exercico01 {

	public static void main(String[] args) {
		String product1 = "Computer";
		String product2 = "Office desk";
		
		int age = 30;
		int code = 5290;
		char gender = 'F';
		
		double price1 = 2100.0;
		double price2 = 650.50;
		double measure = 53.234567;
		
	
		System.out.printf("Products:%n%s, which price is $ %.2f%n%s, which price is $ %.2f%n", product1, price1, product2, price2);
		System.out.printf("%nRecord: %d years old, code %d and gender:%s%n", age, code, gender);
		System.out.printf("%nMeasue wich eight decial place:%f", measure);
		System.out.printf("%nRouded wich (three decial places):%.2f%n", measure);
		
		Locale.setDefault(Locale.US);
		System.out.printf("US deciamal point: %.3f%n", measure);
		
		
	}

}
